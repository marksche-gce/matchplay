...
CREATE TABLE IF NOT EXISTS matches (
  ...
  -- Bracket progression
  feeds_to_match_id UUID REFERENCES matches(id),
  ...
  CHECK ((winner_player_id IS NOT NULL AND winner_team_id IS NULL) OR (winner_player_id IS NULL AND winner_team_id IS NOT NULL) OR (winner_player_id IS NULL AND winner_team_id IS NULL))
);
...
CREATE POLICY "Anyone can register as player" ON players FOR INSERT WITH CHECK (true);
...
-- Admin policies (assuming organizer role from existing system)
CREATE POLICY "Organizers can manage tournaments" ON tournaments FOR ALL USING (has_role(auth.uid(), 'organizer'::app_role));
CREATE POLICY "Organizers can manage players" ON players FOR ALL USING (has_role(auth.uid(), 'organizer'::app_role));
CREATE POLICY "Organizers can manage teams" ON teams FOR ALL USING (has_role(auth.uid(), 'organizer'::app_role));
CREATE POLICY "Organizers can manage registrations" ON tournament_registrations FOR ALL USING (has_role(auth.uid(), 'organizer'::app_role));
CREATE POLICY "Organizers can manage deadlines" ON round_deadlines FOR ALL USING (has_role(auth.uid(), 'organizer'::app_role));
CREATE POLICY "Organizers can manage matches" ON matches FOR ALL USING (has_role(auth.uid(), 'organizer'::app_role));

-- Allow anyone to update match winners (as per requirement)
CREATE POLICY "Anyone can update match winners" ON matches 
  FOR UPDATE USING (true) 
  WITH CHECK (
    (OLD.winner_player_id IS NULL AND OLD.winner_team_id IS NULL) OR 
    (NEW.winner_player_id = OLD.winner_player_id AND NEW.winner_team_id = OLD.winner_team_id)
  );
